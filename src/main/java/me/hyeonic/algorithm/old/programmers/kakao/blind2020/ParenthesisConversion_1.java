...
public class ParenthesisConversion_1 {

    static boolean isCorrectParenthesis(String s) {
        int count = 0;
        for (char c : s.toCharArray()) {
            count += (c == '(') ? 1 : -1;
            if (count < 0) return false; // 올바른 괄호는 절대 음수가 나올 수 없다.
        }
        return count == 0;
    }
    ...
    static String getCorrectParenthesis(String parenthesis) {
        ...
        if (parenthesis.equals("")) {
            ...
            for (int i = 0; i <= parenthesis.length(); i++) {
                count += (parenthesis.charAt(i) == '(') ? 1 : -1;
                if (count == 0) {
                    index = i;
                    break;
                }
            ...
            if (isCorrectParenthesis(u)) {
                ...
                StringBuilder stringBuilder = new StringBuilder("(")
                        ...
                        .append(getCorrectParenthesis(v))
                        /** 4-3. ')'를 다시 붙인다. */
                        .append(")")
                        /** 4-4. u의 첫 번째와 마지막 문자를 제거하고, 나머지 문자열의 괄호 방향을 뒤집어서 뒤에 붙인다. */
                        .append(new StringBuilder(u)
                                .deleteCharAt(u.length() - 1)
                                .deleteCharAt(0)
                                .toString()
                                .replace('(', '_')
                                .replace(')', '(')
